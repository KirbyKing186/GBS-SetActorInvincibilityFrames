...
void actors_handle_player_collision(void) BANKED {
    if ((custom_iframes < player_iframes) || (player_iframes == 0 && player_collision_actor != NULL)) {
        if (player_collision_actor->collision_group) {
            ...
            // Set player to be invincible for N frames
            player_iframes = custom_iframes;
        }
